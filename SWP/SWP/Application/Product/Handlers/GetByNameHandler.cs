...
namespace SWPApi.Application.Product.Handlers
{
    public class GetByNameHandler : IRequestHandler<GetByNameCommand, GetByNameResponse>
    {
        ...
        public async Task<GetByNameResponse> Handle(GetByNameCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var products = _unitOfWork.ProductRepository.Find(x => string.IsNullOrEmpty(request.Name) || x.Name.Contains(request.Name) || x.MilkBrand.Name.Contains(request.Name) || x.MilkBrand.Company.Name.Contains(request.Name) , request.PageNumber, request.PageSize);

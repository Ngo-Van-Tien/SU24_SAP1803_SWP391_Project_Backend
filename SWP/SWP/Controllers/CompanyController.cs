...
namespace SWPApi.Controllers
{
    ...
    public class CompanyController : ControllerBase
    {
        ...
        [Authorize(Roles = UserRolesConstant.AdminOrStaff)]
        [HttpPost]
        public async Task<IActionResult> unlockcompany([FromForm] UnLockCompanyCommand command)
        {
            var result = await _mediator.Send(command);
            if (!result.IsSuccess)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> getproductsbycompany(GetProductByCompanyCommand command)
        {
            var result = await _mediator.Send(command);
            if (!result.IsSuccess)
            {
